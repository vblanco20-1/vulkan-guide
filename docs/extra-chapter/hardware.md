...
There is the ALU (Arithmetic Logic Unit), which implements math operations like addition, substraction, or comparing numbers. 
The Register bank is what stores the "working memory" of the processor. You can think of this as the absolute highest performant memory, but its very small, often just a few numbers worth of space. Operations in a CPU work only on registers for the most part, and its connected directly to the ALU and other core parts of the CPU
To store more data, RAM memory is used. The cpu can load data from this memory into a register, and also can store data from a register into the memory. The common flow of a program is that it will load memory, execute some math on it using the ALU, and store the results.
The whole system is controlled from a Decoder, also known as Frontend, or many other names. This is the part of a cpu controls the main execution of the system. When running an instruction, this part of the cpu will read the binary code of the instruction, and trigger the different sections of the CPU to do their work. This part will often have a special register commonly known as PC(program counter) which controls what is the instruction to load next from memory. In the case the program uses branches or jumps, this program counter can be directly adressed and modified. 

Modern cpus will also have multiple of these cores to be able to do more work at once. A set of instructions is generally known as a "thread" of execution, and each of the CPU cores will be independent from the others. There are ways of syncronize information between the cores using atomic instructions, and there are also ways of syncronizing memory across the cores. 
...
Other source is Wikichip which can have good information on the details of different processors.
